...
// * - the class name of a {@link ForkJoinPool.ForkJoinWorkerThreadFactory}.
...
// * The {@linkplain ClassLoader#getSystemClassLoader() system class loader}
// * is used to load this class.
// * <li>{@code ForkJoinPool.common.maximumSpares}
...
//         */
//        final void topLevelExec(ForkJoinTask<?> t, ForkJoinPool.WorkQueue q, int n) {
...
//                PHASE = l.findVarHandle(ForkJoinPool.WorkQueue.class, "phase", int.class);
//                BASE = l.findVarHandle(ForkJoinPool.WorkQueue.class, "base", int.class);
//                TOP = l.findVarHandle(ForkJoinPool.WorkQueue.class, "top", int.class);
...
//     * "ForkJoinPool.common.maximumSpares" system
...
//    ForkJoinPool.WorkQueue[] workQueues;              // main registry
...
//        ForkJoinPool.ForkJoinWorkerThreadFactory fac = factory;
//        Throwable ex = null;
...
//        ForkJoinPool.WorkQueue w = new ForkJoinPool.WorkQueue(this, wt);
//        if (prefix != null) {
//            synchronized (prefix) {
//                ForkJoinPool.WorkQueue[] ws = workQueues; int n;
...
//                    else {                              // expand array
//                        int an = n << 1;
//                        ForkJoinPool.WorkQueue[] as = new ForkJoinPool.WorkQueue[an];
//                        as[tid] = w;
//                        int am = an - 1;
//                        for (int j = 0; j < n; ++j) {
//                            ForkJoinPool.WorkQueue v;                // copy external queue
...
//            if (lock != null) {
//                synchronized (lock) {
//                    ForkJoinPool.WorkQueue[] ws; int n, i;         // remove index from array
...
//        for (;;) {
//            long c; int sp; ForkJoinPool.WorkQueue[] ws; int i; ForkJoinPool.WorkQueue v;
...
//     */
//    private int tryCompensate(ForkJoinPool.WorkQueue w) {
//        int t, n, sp;
//        long c = ctl;
//        ForkJoinPool.WorkQueue[] ws = workQueues;
//        if ((t = (short)(c >>> TC_SHIFT)) >= 0) {
//            if (ws == null || (n = ws.length) <= 0 || w == null)
//                return 0;                        // disabled
//            else if ((sp = (int)c) != 0) {       // replace or release
//                ForkJoinPool.WorkQueue v = ws[sp & (n - 1)];
//                int wp = w.phase;
...
//                for (int i = 1; i < n; i += 2) {
//                    ForkJoinPool.WorkQueue q; Thread wt; Thread.State ts;
//                    if ((q = ws[i]) != null) {
//                        if (q.source == 0) {
//                            unstable = true;
//                            break;
...
//     * possibly ran task(s).
//     */
//    private boolean scan(ForkJoinPool.WorkQueue w, int r) {
//        ForkJoinPool.WorkQueue[] ws; int n;
//        if ((ws = workQueues) != null && (n = ws.length) > 0 && w != null) {
//            for (int m = n - 1, j = r & m;;) {
//                ForkJoinPool.WorkQueue q; int b;
...
//     * @return task status on exit
//     */
//    final int awaitJoin(ForkJoinPool.WorkQueue w, ForkJoinTask<?> task, long deadline) {
//        int s = 0;
...
//            ForkJoinTask<?> localTask; ForkJoinPool.WorkQueue[] ws;
...
//
//    /**
//     * 将给定的任务提交到提交则“现在队列的提交队列中”，如果队列为null或者被竞争，则创建一个队列。
//     * Adds the given task to a submission queue at submitter's
//     * current queue, creating one if null or contended.
//     *
//     * @param task the task. Caller must ensure non-null. 被提交的任务必须保证非空
//     */
...
//                q = new ForkJoinPool.WorkQueue(this, null);
//                q.array = qa;
//                q.id = qid;
//                q.source = QUIET;
//                if (lock != null) {     // unless disabled, lock pool to install
//                    synchronized (lock) {
//                        ForkJoinPool.WorkQueue[] vs; int i, vn;
...
//        Thread t; ForkJoinWorkerThread w; ForkJoinPool.WorkQueue q;
//        if (task == null)
...
//        int r = ThreadLocalRandom.getProbe();
//        ForkJoinPool.WorkQueue[] ws; ForkJoinPool.WorkQueue w; int n;
...
//     * @return task status on exit
//     */
//    final int helpComplete(ForkJoinPool.WorkQueue w, CountedCompleter<?> task,
//                           int maxTasks) {
...
//        Thread t; ForkJoinWorkerThread wt; ForkJoinPool pool; ForkJoinPool.WorkQueue q;
//        if (((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) &&
...
//            for (long oldSum = 0L;;) {            // repeat until stable
//                ForkJoinPool.WorkQueue[] ws; ForkJoinPool.WorkQueue w;
//                long checkSum = ctl;
...
//     * ForkJoinPool.ForkJoinWorkerThreadFactory, UncaughtExceptionHandler, boolean,
...
//     * #ForkJoinPool(int, ForkJoinPool.ForkJoinWorkerThreadFactory,
...
//     * threads not blocked by a join or {@link ForkJoinPool.ManagedBlocker}.  To
...
//     * ForkJoinPool.ManagedBlocker}, but cannot be replaced because the
...
//        this.workQueues = new ForkJoinPool.WorkQueue[n];
...
//        int parallelism = -1;
//        ForkJoinPool.ForkJoinWorkerThreadFactory fac = null;
...
//            fac = (ForkJoinPool.ForkJoinWorkerThreadFactory) newInstanceFromSystemProperty(
...
//                fac = new ForkJoinPool.InnocuousForkJoinWorkerThreadFactory();
//        }
...
//                return false;
//            else {
//                ForkJoinPool.WorkQueue[] ws; ForkJoinPool.WorkQueue v;
//                if ((ws = workQueues) != null) {
//                    for (int i = 1; i < ws.length; i += 2) {
//                        if ((v = ws[i]) != null) {
//                            if (v.source > 0)
...
//        ForkJoinPool.WorkQueue[] ws; ForkJoinPool.WorkQueue w; ForkJoinTask<?> t;
//        VarHandle.acquireFence();
//        int count = 0;
//        if ((ws = workQueues) != null) {
//            for (int i = 0; i < ws.length; ++i) {
...
//     * thread invoking {@link ForkJoinPool#managedBlock(ForkJoinPool.ManagedBlocker)}.
...
//     * thread is blocked in {@link ForkJoinPool.ManagedBlocker#block blocker.block()}.
//     *
...
//     */
//    static void helpAsyncBlocker(Executor e, ForkJoinPool.ManagedBlocker blocker) {
//        if (e instanceof ForkJoinPool) {
//            ForkJoinPool.WorkQueue w; ForkJoinWorkerThread wt; ForkJoinPool.WorkQueue[] ws; int r, n;
...
//                    ("ForkJoinPool.common.maximumSpares");
//            if (p != null)
